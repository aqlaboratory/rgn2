...
#Modification: functions for C_Alpha geomtry
...
def torsion_and_curvature_to_rotation_translation(parameters, r=C_ALPHA_LENGTH, name=None):
    ...
    with tf.name_scope(name, 'torsion_and_curvature_to_rotation_translation', [parameters]) as scope:
        ...
        # r_0 = tf.ones_like(sx) * r
        # 
        #tf.zeros(num_steps, batch_size)
...
#Modification: for novel geomtry

def simple_static_rotation_translation_to_coordinate(rotation_translation, max_num_steps = 300, name=None):
    ...
    with tf.name_scope(name, 'simple_static_rotation_translation_to_coordinate', [rotation_translation]) as scope:
        ...
        # [NUM_STEPS + 1, BATCH_SIZE, 4, 3]

        frames = tf.convert_to_tensor(frames, name='frames')

        coords = tf.squeeze(frames[1:,:,-1])
...
def simple_dynamic_rotation_translation_to_coordinate(rotation_translation, parallel_iterations=1, swap_memory=False, name=None):
    ...
    with tf.name_scope(name, 'simple_dynamic_rotation_translation_to_coordinate', [rotation_translation]) as scope:
        ...
        # extension function
        def extend(frames, rotation_translation):
            coord = tf.matmul(rotation_translation, frames[-1]) 
            return coord

        #Make sure you're only keeping the C_Alpha coordinate    
        ...
        def body(i, frames, coords_ta): # (NUM_STEPS x NUM_DIHEDRALS) x [BATCH_SIZE, NUM_DIMENSIONS] 
            coord = extend(frames, rotation_translation[i - 1])
            return [i + 1, frames, coords_ta.write(i, coord)]
