...
#import tensorflow_graphics.geometry.transformation as tfg_transformation  #Modification, includes geometrical functions for rotations and quaternions
...
#from tensorflow_graphics import geometry   #Modification, includes geometrical functions for rotations and quaternions
...
class GeomNetModel(object):
    ...
    def _create_graph(self, mode, config):
        ...
        with tf.variable_scope(SCOPE, reuse=(mode == 'evaluation')) as scope, \
            ...
            #Modification: option for multiple networls, transformers, RNNs, maybe CNNs (for later)
...
def _alphabet(mode, config):
    ...
    alphabet = tf.get_variable(name='alphabet',
                               shape=[config['alphabet_size'], config['number_parametrization']], #Modification: generalized parametrization
...
#Generalize _dihedrals function to include different geometrical models describing protein geomtry: dihedrals, quaternions, Euler-angles, etc.
...
def _geometric_parametrization(mode, config, inputs, alphabet=None):
    ...
    if nonlinear_out_proj_size is not None:
        ...
        for idx, (layer_size, init) in enumerate(zip(nonlinear_out_proj_size, config['recurrent_nonlinear_out_proj_init'])):
            ...
            outputs = layers.fully_connected(outputs, layer_size, scope='nonlinear_parameters_' + str(idx), 
                                             ...
                                             outputs_collections=config['name'] + '_' + tf.GraphKeys.ACTIVATIONS, 
                                             variables_collections={'weights': [tf.GraphKeys.WEIGHTS], 'biases': [tf.GraphKeys.BIASES]})
        parameter_inputs = outputs  # Modification dihedral_inputs to parameters_input
        # [NUM_STEPS, BATCH_SIZE, NONLINEAR_DIHEDRALS_LAYER_SIZE]
    else:
        parameter_inputs = inputs  # Modification dihedral_inputs to parameters_input
    ...
    if config['is_alphabetized']:
        ...
        #Modification: to account for quaternion: define normalization function. I could introduce other functions for other parametrization
        #if config['is_quaternion']: alphabet = tfg.geometry.transformation.quaternion.normalize(alphabet, eps=1e-12,name=None)
        ...
        # Modification: replace dihedrals by parameters
        num_steps = tf.shape(linear)[0]
        batch_size = linear.get_shape().as_list()[1]
        flattened_parameters = reduce_mean_angle(probs, alphabet)                            # [NUM_STEPS x BATCH_SIZE, NUM_DIHEDRALS]
        parameters = tf.reshape(flattened_parameters, [num_steps, batch_size, config['number_parametrization']]) # [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]

        # Modification for later
        #if config['model_parametrization'] == config['dihedrals']:
        #   flattened_parameters = reduce_mean_angle(probs, alphabet)                            # [NUM_STEPS x BATCH_SIZE, NUM_DIHEDRALS]
        #   parameters = tf.reshape(flattened_parameters, [num_steps, batch_size, config['number_parametrization']]) # [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]

        #if config['model_parametrization'] == config['quaternions']:
        #   flattened_parameters = reduce_mean_quaternion(probs, alphabet)   
        #   parameters = tf.reshape(flattened_parameters, [num_steps, batch_size, config['number_parametrization']])
    else:
        # just linear
        parameters = linear
...
def _coordinates(config, parameters):
    ...
    #converts angles to points ready for reconstruction.
    ...
    #                                           parallel_iterations=config['num_reconstruction_parallel_iters']) 
                  #[NUM_STEPS x NUM_DIHEDRALS, BATCH_SIZE, NUM_DIMENSIONS]
    #option: define such option in the function angle_to_point, in geom_ops.py

    #Modification: converts angles (dihedrals, torsion and curvatures, etc.) to final 3D coordinates.
    ...
    for case in switch(config['angle_type']):
        if case('dihedrals'):
            ...
            coordinates = simple_static_rotation_translation_to_coordinate(rotation_translation) #[NUM_STEPS, BATCH_SIZE, NUM_DIMENSIONS]
        else:
