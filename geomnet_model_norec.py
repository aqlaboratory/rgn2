            ...
            if config.loss['include']:
                ...
                for group_id, group_filter in filters.iteritems():
                    with tf.variable_scope(group_id):
                        ...
                        if effective_secondary_loss == 0 and effective_tertiary_loss == 0:
                            loss = tf.reduce_sum(recurrent_outputs)
                        else:
...
def _recurrence(mode, config, inputs, num_stepss):
    ...
    if config['recurrent_init'] is not None:
    ...
    if 'Cudnn' in config['recurrent_unit']:
        ...
        if config['bidirectional']:
            outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=_recurrent_cell(mode, config, recurrent_init, 'fw'), 
            ...
            outputs, _ = tf.nn.dynamic_rnn(              cell=_recurrent_cell(mode, config, recurrent_init),
